    ...
    <table style="border-collapse: collapse; width: 100%;">
      <tbody>
        ...
        <tr>
          ...
          <td style="padding: 8px;"><a href="mailto:${safe(data.email)}">${safe(data.email)}</a></td>
        </tr>
        ${data.phone ? `<tr><td style=\"padding: 8px; font-weight: 600;\">Phone</td><td style=\"padding: 8px;\">${safe(data.phone)}</td></tr>` : ""}
        ${data.subject ? `<tr><td style=\"padding: 8px; font-weight: 600;\">Subject</td><td style=\"padding: 8px;\">${safe(data.subject)}</td></tr>` : ""}
      </tbody>
    </table>

    <div style="margin-top: 20px;">
      <div style="font-weight: 600; margin-bottom: 8px;">Message</div>
      <div style="white-space: pre-wrap; background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 8px; padding: 12px;">${safe(data.message)}</div>
    </div>
  </div>
  `
}
