...
import { renderContactEmailHtml, renderContactConfirmationEmailHtml } from "@/lib/email/contact"
...
export async function POST(req: Request) {
  try {
    ...
    const { name, email, phone, subject, message, agreementChecked } = body || {}

    const errors: string[] = []
    if (!name || typeof name !== "string") errors.push("name is required")
    if (!email || typeof email !== "string" || !validateEmail(email)) errors.push("valid email is required")
    ...
    const html = renderContactEmailHtml({ name, email, phone, subject, message })
    ...
    const confirmationHtml = renderContactConfirmationEmailHtml({ name, email, phone, subject, message })
