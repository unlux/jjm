  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif; padding: 24px; color: #111827;">
    ...
    <table style="border-collapse: collapse; width: 100%;">
      <tbody>
        ...
        <tr><td style="padding:8px; font-weight:600;">Phone</td><td style="padding:8px;">${safe(data.phone)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Age</td><td style="padding:8px;">${safe(data.age)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Gender</td><td style="padding:8px;">${safe(data.gender)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Traits</td><td style="padding:8px;">${safe((data.trait || []).join(", "))}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Skills</td><td style="padding:8px;">${safe((data.skills || []).join(", "))}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Happy</td><td style="padding:8px;">${safe(data.happy)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Interests</td><td style="padding:8px;">${safe(data.interests)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Struggles</td><td style="padding:8px;">${safe((data.struggles || []).join(", "))}</td></tr>
      </tbody>
    </table>
    ${data.extra?.trim() ? `<div style="margin-top:20px;"><div style="font-weight:600; margin-bottom:8px;">Additional Notes</div><div style="white-space: pre-wrap; background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 8px; padding: 12px;">${safe(
      data.extra
    )}</div></div>` : ""}
  </div>`
}
  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif; padding: 24px; color: #111827;">
    ...
    <div style="margin-top: 20px;">
      ...
      <table style="border-collapse: collapse; width: 100%;"><tbody>
        <tr><td style="padding:8px; font-weight:600; width:160px;">Email</td><td style="padding:8px;">${safe(data.email)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Phone</td><td style="padding:8px;">${safe(data.phone)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Age</td><td style="padding:8px;">${safe(data.age)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Gender</td><td style="padding:8px;">${safe(data.gender)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Traits</td><td style="padding:8px;">${safe((data.trait || []).join(", "))}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Skills</td><td style="padding:8px;">${safe((data.skills || []).join(", "))}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">What makes your kiddo happy</td><td style="padding:8px;">${safe(data.happy)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Interests</td><td style="padding:8px;">${safe(data.interests)}</td></tr>
        <tr><td style="padding:8px; font-weight:600;">Struggles</td><td style="padding:8px;">${safe((data.struggles || []).join(", "))}</td></tr>
      </tbody></table>
    </div>
...
export async function POST(req: Request) {
  try {
    ...
    const errors: string[] = []
    if (!email || typeof email !== "string" || !/.+@.+\..+/.test(email)) errors.push("valid email is required")
    if (!phone || typeof phone !== "string" || !/^\d{10}$/.test(phone)) errors.push("valid 10-digit phone is required")
    ...
    const subject = `[Custom Kit] Request from ${email}${age ? ` (Age ${age})` : ""}`
